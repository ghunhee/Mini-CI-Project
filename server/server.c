...
/* ── 4-A: 레이아웃 검증 (FIX-7) ─────────────────────────────────
 * CommonHeader 와 각 패킷 헤더의 공통 필드 오프셋이 동일함을 보장.
 * 이 assert 가 실패하면 즉시 컴파일 에러가 나므로 런타임 오염 없음.
 * ─────────────────────────────────────────────────────────────── */
#include <assert.h>

/* CommonHeader 정의 (server.c 에 아직 없다면 여기에 추가) */
...
_Static_assert(sizeof(CommonHeader) == 8,
    "CommonHeader 크기가 8바이트여야 합니다");

_Static_assert(offsetof(SubmitHeader,   magic) == offsetof(CommonHeader, magic),
    "SubmitHeader.magic 오프셋 불일치");
_Static_assert(offsetof(SubmitHeader,   pkt_type) == offsetof(CommonHeader, pkt_type),
    "SubmitHeader.pkt_type 오프셋 불일치");
_Static_assert(offsetof(GetFilesHeader, magic) == offsetof(CommonHeader, magic),
    "GetFilesHeader.magic 오프셋 불일치");
_Static_assert(offsetof(GetFilesHeader, pkt_type) == offsetof(CommonHeader, pkt_type),
    "GetFilesHeader.pkt_type 오프셋 불일치");
_Static_assert(offsetof(FileMetaHeader, magic) == offsetof(CommonHeader, magic),
    "FileMetaHeader.magic 오프셋 불일치");
_Static_assert(offsetof(FileChunkHeader,magic) == offsetof(CommonHeader, magic),
    "FileChunkHeader.magic 오프셋 불일치");
_Static_assert(offsetof(FileEndHeader,  magic) == offsetof(CommonHeader, magic),
    "FileEndHeader.magic 오프셋 불일치");
_Static_assert(offsetof(FileErrorHeader,magic) == offsetof(CommonHeader, magic),
    "FileErrorHeader.magic 오프셋 불일치");
...
static void build_and_send_result(int          cli_fd,
    ...
    strncpy(resp.week_name,  task_hw,        MAX_WEEK_NAME);

    /* Record before sending so the client receives this student's attempt count. */
    pthread_rwlock_rdlock(&g_ctx.hw_lock);
    if (strncmp(g_ctx.current_hw, task_hw, MAX_WEEK_NAME) == 0) {
        my_submit_count = ctx_record_submission(&g_ctx, hdr.student_id, grade);
        ctx_write_csv(&g_ctx, hdr.student_id, grade, exec_ms, mem_kb, task_hw);
    } else {
        fprintf(stderr, "[warn] week changed during grading (%s -> %s); record skipped.\n",
                task_hw, g_ctx.current_hw);
    }
    ...
    resp.student_submit_count = htons(my_submit_count);
 ...
 * 반환값    : 0 성공, -1 실패
 * ─────────────────────────────────────────────────────────────── */
#define MAX_FILE_SIZE_BYTES (100UL * 1024 * 1024)  /* 100 MB */
...
static int send_one_file(int         cli_fd,
    ...
    /* FIX-8: 100MB 상한 체크 */
    if ((uint64_t)st.st_size > MAX_FILE_SIZE_BYTES) {
        fprintf(stderr, "[pull] 파일 크기 초과 (최대 100MB): %s\n", file_path);
        close(fd); return -1;
    }
    /* 빈 파일도 전송은 허용 (fsize=0) */
    ...
    if (!send_ok) return -1;   /* FIX-2: 청크 전송 실패 → 호출자에게 알림 */
    ...
    end.checksum    = htonl(~crc);

    return _send_all(cli_fd, &end, sizeof(end));
}

/* ── 4-C: handle_file_pull() 전체 교체 (FIX-2) ──────────────────
 *
 * FIX-2: send_one_file() 실패 시 RES_FILE_ERROR 를 클라이언트에
 *        전송한 뒤 함수 종료. 기존 코드는 그냥 return 해서
 *        클라이언트가 파일 누락을 알지 못했음.
 * ─────────────────────────────────────────────────────────────── */

/* 내부 헬퍼: RES_FILE_ERROR 전송 + 로그 */
static void send_pull_error(int cli_fd, const char *fmt, ...) {
    FileErrorHeader err;
...
void handle_file_pull(int cli_fd, const char *student_id) {
    ...
    if (!dir) {
        send_pull_error(cli_fd,
            "배포 폴더 없음: %.*s (교수님께 문의)", MAX_ERR_PATH, dist_dir);
        return;
    }

    /* 3) 파일 목록 수집 */
    char file_paths[MAX_DIST_FILES][1024];
    ...
    while ((ent = readdir(dir)) != NULL && file_count < MAX_DIST_FILES) {
        ...
        char full[1024];
        snprintf(full, sizeof(full), "%s/%s", dist_dir, ent->d_name);

        struct stat st;
        if (stat(full, &st) < 0 || !S_ISREG(st.st_mode)) continue;

        strncpy(file_paths[file_count], full,        1023);
        strncpy(file_names[file_count], ent->d_name, MAX_FILENAME - 1);
        file_paths[file_count][1023]           = '\0';
    ...
    if (file_count == 0) {
        send_pull_error(cli_fd,
            "배포 파일 없음: %.*s (아직 업로드 전)", MAX_ERR_PATH, dist_dir);
        return;
    }
    ...
    /* 5) 파일별 전송
     * FIX-2: 실패 시 에러 패킷 전송 후 즉시 종료 */
    for (int i = 0; i < file_count; i++) {
        if (send_one_file(cli_fd,
                          file_paths[i], file_names[i],
                          (uint8_t)(i + 1), (uint8_t)file_count) < 0) {
            /* 클라이언트 연결이 살아있다면 에러 알림 */
            send_pull_error(cli_fd,
                "파일 전송 실패: %.*s",
                MAX_FILENAME - 1, file_names[i]);
    ...
    fprintf(stderr, "[pull] 완료: student=%s  %d개 파일\n",
...
void handle_client(int client_sock, int thread_id) {
    ...
    char task_hw[MAX_WEEK_NAME];
    pthread_rwlock_rdlock(&g_ctx.hw_lock);
    strncpy(task_hw, g_ctx.current_hw, sizeof(task_hw));
    pthread_rwlock_unlock(&g_ctx.hw_lock);
    ...
    close(pipefd[0]);
    build_and_send_result(client_sock, fheader, final_res, final_exec, final_mem, task_hw);
