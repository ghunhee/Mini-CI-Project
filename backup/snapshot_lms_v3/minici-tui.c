...
#define ACT_IPC    0
#define ACT_DL     1   /* 제출 기한 설정  */
#define ACT_TSSET  2   /* test.sh 경로 설정 */
#define ACT_TSDIST 3   /* test.sh 배포    */
    ...
    while(1) {
        ...
        if(ch=='\n'||ch==KEY_ENTER) {
            ...
            if(m->action==ACT_TSSET) {
                char picked[1024] = {0};
                if(popup_file_picker(picked, sizeof(picked)) == 0) {
                    strncpy(g_testsh, picked, sizeof(g_testsh)-1);
                    snprintf(msg, sizeof(msg), "[설정] test.sh 경로: %s", g_testsh);
                    log_msg(2, msg);
                } else {
                    log_msg(4, "[취소] 파일 선택을 취소했습니다.");
                }
                continue;
            }
            if(m->action==ACT_TSDIST) {
                distribute_testsh(); continue;
            }
