...
static int ensure_dir(const char *path) {
    ...
    if (mkdir(path, 0755) < 0 && errno != EEXIST) { perror("[ctx] mkdir"); return -1; }
    return 0;
}
...
void ctx_init(ServerContext *ctx) {
    ...
    pthread_mutex_lock(&ctx->csv_lock);
    {
        ...
        ctx->csv.csv_fd = open(CSV_PATH, O_WRONLY | O_CREAT | O_APPEND, 0644);
        if (ctx->csv.csv_fd < 0) {
            perror("[ctx] open results.csv");
        } else if (need_header) {
            const char *hdr = "Timestamp,Student_ID,Assignment_Week,Result,ExecTime_ms,Memory_KB\n";
...
void ctx_record_submission(ServerContext *ctx, const char *student_id, GradeResult result) {
    ...
    for (int i = 0; i < MAX_STUDENTS; i++) {
        ...
        if (strncmp(ctx->students[i].student_id, student_id, MAX_STUDENT_ID) == 0) {
            found = &ctx->students[i]; break;
        }
    }
    if (!found) {
        for (int i = 0; i < MAX_STUDENTS; i++) {
            if (ctx->students[i].student_id[0] == '\0') {
                ...
                found->submitted = 0; found->best_result = GRADE_WA; found->submit_count = 0;
                break;
            }
        }
    }
    if (!found) { pthread_mutex_unlock(&ctx->tracker_lock); return; }
    found->submit_count++;
    found->last_submit_ts = time(NULL);
    if (result == GRADE_AC && !found->submitted) {
        found->submitted = 1; found->best_result = GRADE_AC;
        ctx->submitted_count++;
        fprintf(stderr, "[tracker] %s AC! (%u / %u)\n", student_id, ctx->submitted_count, ctx->total_students);
...
void ctx_write_csv(ServerContext *ctx, const char *student_id, GradeResult result, uint32_t exec_ms, uint32_t mem_kb) {
    ...
    char ts_buf[32]; time_t now = time(NULL); struct tm *tm = localtime(&now);
    strftime(ts_buf, sizeof(ts_buf), "%Y-%m-%dT%H:%M:%S", tm);
    char week_snap[MAX_WEEK_NAME];
    pthread_rwlock_rdlock(&ctx->hw_lock);
    strncpy(week_snap, ctx->current_hw, MAX_WEEK_NAME - 1); week_snap[MAX_WEEK_NAME - 1] = '\0';
    pthread_rwlock_unlock(&ctx->hw_lock);
    char line[512];
    int len = snprintf(line, sizeof(line), "%s,%s,%s,%s,%u,%u\n", ts_buf, student_id, week_snap, grade_str(result), exec_ms, mem_kb);
    pthread_mutex_lock(&ctx->csv_lock);
    ssize_t w = write(ctx->csv.csv_fd, line, len);
    if (w < 0) perror("[ctx] csv write");
    else { ctx->csv.total_records++; ctx->csv.last_write_ts = now; }
    pthread_mutex_unlock(&ctx->csv_lock);
}
