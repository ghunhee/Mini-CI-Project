...
static struct {
    ...
    uint16_t my_submit_count;
} g_status = { "week_1", 0, 0, "---", 0, 0 };
...
static void draw_status_bar(void) {
    ...
    if (g_status.my_submit_count == 0) {
        snprintf(try_str, sizeof(try_str), "--");
    } else {
        snprintf(try_str, sizeof(try_str), "#%u", g_status.my_submit_count);
    }
    ...
    snprintf(right, sizeof(right),
             "Try: %-5s | Result: %-4s  %ums ",
             try_str, g_status.last_result, g_status.last_exec_ms);
...
static void on_result_received(const ResultHeader *r) {
    ...
    g_status.week[MAX_WEEK_NAME - 1] = '\0';
    g_status.submitted    = ntohs(r->submitted_count);
    g_status.total        = ntohs(r->total_students);
    strncpy(g_status.last_result, gnames[gr], sizeof(g_status.last_result) - 1);
    g_status.last_result[sizeof(g_status.last_result) - 1] = '\0';
    g_status.last_exec_ms = ntohl(r->exec_time_ms);
    g_status.my_submit_count = ntohs(r->student_submit_count);
}
...
static void jump_to_editor(const char *filepath, int line_number) {
    ...
    if (pid == 0) {
        ...
        char *args[] = {"cmd.exe", "/c", "start", "/wait", "wsl", "vi",
...
/* ── 5-A: 레이아웃 검증 (FIX-7) ───────────────────────────────── */
#include <assert.h>
_Static_assert(sizeof(CommonHeader) == 8, "CommonHeader 크기가 8바이트여야 합니다");
_Static_assert(offsetof(SubmitHeader,   magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
_Static_assert(offsetof(GetFilesHeader, magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
_Static_assert(offsetof(FileMetaHeader, magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
_Static_assert(offsetof(FileChunkHeader,magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
_Static_assert(offsetof(FileEndHeader,  magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
_Static_assert(offsetof(FileErrorHeader,magic) == offsetof(CommonHeader, magic), "오프셋 불일치");
...
/* ── 5-B: 경로 조작 공격(Path Traversal) 검사기 (FIX-1) ─────────
 * 클라이언트 저장 경로를 오염시킬 수 있는 파일명 필터링
 * ─────────────────────────────────────────────────────────────── */
...
static int do_pull_mode(void) {
    ...
    while (1) {
        ...
        case RES_FILE_META: {
            ...
            if (_recv_all(sock, (uint8_t *)&meta + sizeof(CommonHeader), rest) < 0)
                goto recv_error;

            meta.filename[MAX_FILENAME - 1] = '\0';

            /* FIX-1: 수신한 파일명 안전성 검증 */
            if (!is_safe_filename(meta.filename)) {
                fprintf(stderr, "\n  [보안 경고] 서버가 위험한 파일명을 보냈습니다: %s\n",
                        meta.filename);
                goto recv_error;
            }
            ...
            if (out_fd < 0) {
                fprintf(stderr, "  [오류] 파일 생성 실패: %s — %s\n",
            ...
            fprintf(stdout, "  ▼ %-30s  %u bytes", meta.filename, fsize);
            fflush(stdout);
            break;
        }
        ...
        /* ── RES_FILE_CHUNK ── */
        case RES_FILE_CHUNK: {
            if (state != PULL_STATE_WAIT_CHUNK || out_fd < 0) goto recv_error;

            if (hdr_size != sizeof(FileChunkHeader)) goto recv_error;
            FileChunkHeader chk;
            uint16_t rest = hdr_size - sizeof(CommonHeader);
            if (_recv_all(sock, (uint8_t *)&chk + sizeof(CommonHeader), rest) < 0)
                goto recv_error;
            ...
            for (uint32_t i = 0; i < csz; i++) {
                crc_accum ^= buf[i];
                for (int j = 0; j < 8; j++)
                    crc_accum = (crc_accum >> 1) ^ (0xEDB88320u & -(crc_accum & 1u));
            }
            ...
            while (written < csz) {
                ...
                if (w <= 0) { free(buf); goto recv_error; }
                written += w;
            }
            free(buf);
        ...
        case RES_FILE_END: {
            ...
            FileEndHeader end;
            uint16_t rest = hdr_size - sizeof(CommonHeader);
            if (_recv_all(sock, (uint8_t *)&end + sizeof(CommonHeader), rest) < 0)
                goto recv_error;
        ...
        /* ── RES_FILE_ERROR (FIX-2) ── */
        case RES_FILE_ERROR: {
            if (hdr_size != sizeof(FileErrorHeader)) goto recv_error;
            FileErrorHeader err;
            uint16_t rest = hdr_size - sizeof(CommonHeader);
            if (_recv_all(sock, (uint8_t *)&err + sizeof(CommonHeader), rest) == 0) {
...
int main(int argc, char *argv[]) {
    ...
    int selected = 0, ch = 0;
    char key_hint[128] = "Keys: UP/DOWN move | ENTER open editor | q quit";
    ...
    while (1) {
        ...
        mvwprintw(list_win, getmaxy(list_win) - 2, 2,
                  "%.*s", max_x - 4, key_hint);
        ...
        mvwprintw(detail_win, getmaxy(detail_win) - 2, 2,
                  "%.*s", max_x - 4,
                  "Tip: press q to quit, ENTER to edit and rejudge.");
        ...
        if (ch == 'q' || ch == 'Q') {
            ...
            selected--;
            snprintf(key_hint, sizeof(key_hint),
                     "Keys: UP/DOWN move | ENTER open editor | q quit");
        } else if (ch == KEY_DOWN && selected < num_files - 1) {
            selected++;
            snprintf(key_hint, sizeof(key_hint),
                     "Keys: UP/DOWN move | ENTER open editor | q quit");
        } else if (ch == '\n' || ch == KEY_ENTER) {
            /* 에디터 점프 전 파일 수정 시간 기록 */
            struct stat st_before, st_after;
            stat(argv[selected + 1], &st_before);

            def_prog_mode();
            endwin();
            jump_to_editor(argv[selected + 1], 1);

            /* 에디터 종료 후 파일 수정 시간 확인 */
            stat(argv[selected + 1], &st_after);
            
            if (st_before.st_mtime != st_after.st_mtime) {
                /* 파일이 수정된 경우에만 재채점 */
                send_source(student_id, argv[selected + 1], &results[selected]);
                printf("\n[파일이 수정되어 재채점되었습니다. 엔터를 누르면 화면으로 돌아갑니다.]\n");
                while (getchar() != '\n');
            }
            ...
            snprintf(key_hint, sizeof(key_hint),
                     "Unknown key. Use UP/DOWN, ENTER, or q.");
        }
    }
